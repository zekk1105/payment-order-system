...
- [ ] **管轄裁判所の判定が不正確**：2026-04-30 本番確認で誤判定が発生。`lib/court-data.ts` のデータ内容・マッチングロジックを見直す必要あり（次のチャットで優先対応）
  - 確認済みの誤判定例：「大阪府・守口市」→「守口簡易裁判所」と表示されるが、**守口簡易裁判所は実在しない**
  - 対応方針：`lib/court-data.ts` の大阪府エントリを精査し、守口市など誤データを含む裁判所名を修正する。最高裁判所ウェブサイト（https://www.courts.go.jp）で実在確認してからデータを修正すること
...
| 高 | 管轄裁判所の判定バグ修正 | 「大阪府・守口市」→「守口簡易裁判所」と誤表示（実在しない）。`lib/court-data.ts` の大阪府データを最高裁サイトで実在確認しながら修正する。同様の誤データが他府県にも存在する可能性あり、全件精査が必要 |
